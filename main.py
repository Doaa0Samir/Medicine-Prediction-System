...
def helper(dis):
    ...
    pre = [col for col in pre.values[0]]  # flatten 2D array to list

    # Medications
    med_series = medications[medications['Disease'] == dis]['Medication']
    med = []
    for item in med_series:
        try:
            med.extend(ast.literal_eval(item))
        except:
            med.append(item)
    ...
    die = []
    for item in die_series:
        try:
            die.extend(ast.literal_eval(item))
        except:
            die.append(item)
    ...
    wrkout = []
    for item in wrkout_series:
        try:
            wrkout.extend(ast.literal_eval(item))
        except:
            wrkout.append(item)
